function Chip(id, x, y) {
	...
	this.cracksLength;
	this.interCracks = [];
	this.interCracksLength = Math.floor(Math.random()*500)+50;
	this.maxCrackGap = 45;
}
...
Chip.prototype.init = function() {
	...
	for (var u = 0; u < this.interCracksLength; u++) {
		...
		var randomCrackId_2 = randomCrackId_1+5;
...
var sky2 = new Image();
    ...
    if (init) {
			init = false;
      setTimeout(function(){
        document.getElementById('canvas').style.backgroundImage = "url("+sky1_src+")";
      },500);
...
function reset() {
	...
	if (phase == 1) {
		sky2.src = sky1_src;
		setTimeout(function() {
			document.getElementById('canvas').style.backgroundImage = "url("+sky2_src+")";
		}, 500);
		phase = 2;
	} else {
		sky2.src = sky2_src;
